...
static int reset(sensor_t *sensor)
{
    ...
    ret = write_regs(sensor->slv_addr, gc0328_sensor_default_regs, sizeof(gc0328_sensor_default_regs)/(sizeof(uint8_t) * 2));
    if (ret == 0) {
...
static int set_pixformat(sensor_t *sensor, pixformat_t pixformat)
{
    ...
    case PIXFORMAT_RGB565:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0x0f, 6);  //RGB565
        break;

    case PIXFORMAT_YUV422:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = set_reg_bits(sensor->slv_addr, 0x44, 0, 0x0f, 2); //yuv422 Y Cb Y Cr
        break;
    case PIXFORMAT_GRAYSCALE:
        write_reg(sensor->slv_addr, 0xfe, 0x00);
        ret = write_reg(sensor->slv_addr, 0x44, 0xb1);
        break;
    default:
