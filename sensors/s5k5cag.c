...
{
    int ret = SCCB_Read_Addr16_Val16(slv_addr, reg);
#ifdef REG_DEBUG_ON
    if (ret < 0) {
...
{
    int ret = SCCB_Read_Addr16_Val16(sensor->slv_addr, reg & 0xFFFF);
    if (ret > 0) {
        ret &= mask;
    }
    return ret;
...
{
    int ret = 0;
    ret = SCCB_Read_Addr16_Val16(sensor->slv_addr, reg & 0xFFFF);
    if(ret < 0){
        return ret;
    }
    value = (ret & ~mask) | (value & mask);
    ret = SCCB_Write_Addr16_Val16(sensor->slv_addr, reg & 0xFFFF, value);
    return ret;
}

static int set_reg_bits(sensor_t *sensor, uint16_t reg, uint8_t offset, uint8_t length, uint16_t value)
{
    int ret = 0;
    ...
    ret = SCCB_Write_Addr16_Val16(sensor->slv_addr, reg, value);
    return ret;
}

static int set_regs(sensor_t *sensor, const uint16_t (*regs)[2], uint32_t regs_entry_len)
{
    int i = 0, ret = 0;
    while (!ret && (i<regs_entry_len)) {
        if (regs[i][0] == REG_DLY) {
            vTaskDelay(regs[i][1] / portTICK_PERIOD_MS);
        } else {
            ret = SCCB_Write_Addr16_Val16(sensor->slv_addr, regs[i][0], regs[i][1]);
        }
        i++;
    }
    return ret;
...
#define WRITE_REG_OR_RETURN(reg, val) ret = set_reg(sensor, reg, 0xFFFF, val); if(ret){return ret;}
...
{
    int ret = set_regs(sensor, regs_common, sizeof(regs_common)/(sizeof(uint16_t) * 2));
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if (framesize == FRAMESIZE_VGA) {
        WRITE_REGS_OR_RETURN(S5K5CAG_VGA, sizeof(S5K5CAG_VGA)/(sizeof(uint16_t) * 2));
    }
